...
import io
import pandas as pd
from PanEL_utils import PanEL_func_utils as pan_el
...
if 'searched_el' not in st.session_state:
    st.session_state.searched_el = False

st.header('NetMHCPanEL 4.1 (IEDB recommended 04.2020)')
st.write('Predicts binding of peptides to any MHC molecule of the known sequence using artificial neural networks (ANNs). The EL data covers 177\
MHC molecules from human (HLA-A, B, C, E), mouse (H-2), cattle (BoLA), primates (Patr, Mamu, Gogo), swine (SLA), equine (Eqca) and dog\
(DLA).')
...
if search or st.session_state.searched_el:
    st.write('---')
    ...
    df = pan_el.get_result_df(protein_seq, allele)
